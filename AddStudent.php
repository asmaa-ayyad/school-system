...
if($_SERVER["REQUEST_METHOD"] =="POST"){
    ...
    if(preg_match($pattern,$_POST["email"]) && is_numeric($_POST["phone"])){
        ...
        $query_check_email = "SELECT * FROM Students WHERE student_email = '$email'";
        $result_check_email = mysqli_query($connecction, $query_check_email);
    
        if ($result_check_email && mysqli_num_rows($result_check_email) > 0) {
            $emailErr = "*email already exists";
        } else {
            $query="INSERT INTO Students (s_firstName, s_lastName ,student_email,student_Password ,student_phone,student_image)
            VALUES('$f_name','$l_name','$email','$password' ,'$phone','$upload_path')";
        $result= mysqli_query($connecction,$query);

        $query2="INSERT INTO user (user_name, password ,role_id)
