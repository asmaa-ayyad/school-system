            ...
            if(mysqli_num_rows($result)>0){
               $row = mysqli_fetch_assoc($result);
                          ...
                          if ($_SERVER['REQUEST_METHOD'] == "POST") {
                            ...
                            if (empty($_POST["f_name"])||empty($_POST["l_name"])|| empty($_POST["email"])||empty($_POST["password"])||empty($_POST["phone"])) {
                                ...
                                move_uploaded_file($file_tmp,$upload_path); 
                                    ...
                                    $upload_path = $row["student_image"];
                                }
                            //2- write query
                            $query = " UPDATE students SET s_firstName = '$fname', s_lastName =' $lname',
                            student_email = '$email', student_phone ='$phone'  , student_password= '$password' ,student_image='$upload_path' WHERE id = $id";
                            //3- implement query
                            $result = mysqli_query($connecction,$query);
                            ...
                            if($result && mysqli_affected_rows($connecction) > 0){
                                $query = "UPDATE USER SET user_name = '$email', 
                                          password = '$password' 
                                          where user_name ='" . $row['student_email'] . "'";
                                $result = mysqli_query($connecction,$query);
...
?>
                <div class="bg-light text-center rounded p-4">
                    <div class="d-flex align-items-center justify-content-between mb-4">
                        <h6 class="mb-0">Edite Student</h6>
  ...
  <div class="row mb-3">
    ...
    <div class="col-sm-9">
      <input type="text" class="form-control form-control-lg" id="inputname3" name="f_name"  
      value="<?= ((isset($row)) ? $row['s_firstName']:'') ?>"
      >
    </div>
  </div>
  <div class="row mb-3">
    ...
    <div class="col-sm-9">
      <input type="text" class="form-control form-control-lg" id="inputname3" name="l_name"
      value="<?= ((isset($row)) ? $row['s_lastName']:'') ?>"
  ...
  <div class="row mb-3">
    ...
    <div class="col-sm-9">
      <input type="Email" class="form-control form-control-lg" id="inputConfirmPassword" name="email"
      
      value="<?= ((isset($row)) ? $row['student_email']:'') ?>"
  ...
  <div class="row mb-3">
    ...
    <div class="col-sm-9">
      <input type="tel" class="form-control form-control-lg" id="inputPhone" name="phone"
      
      value="<?= ((isset($row)) ? $row['student_phone']:'') ?>"
  ...
  <div class="row mb-3">
    ...
    <div class="col-sm-9">
      <input type="password" class="form-control form-control-lg" id="inputPassword3" name="password"
      
      value="<?= ((isset($row)) ? $row['student_Password']:'') ?>"
  ...
  <div class="row mb-3">
    ...
    <div class="col-sm-9">
      <div class="input-group">
        <input type="file" class="form-control form-control-lg" id="inputUsername" aria-label="Recipient's username" name="image">
        
      </div>
      <?php if (!empty($row['teacher_image'])) : ?>
      <img src="<?php echo $row['student_image']; ?>" alt="Student Image" style="max-width: 200px; max-height:200px; margin-top: 10px;">
      <?php endif; ?>
    </div>
  </div>
